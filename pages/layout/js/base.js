...
; (function ($) {
    ...
    $.fn.EditPaging = function (options) {
     ...
     $.fn.jqueryMenu = function (options) {
         ...
         return this.each(function () {
             ...
             self.find("li>ul").css({ "display": "none", "position": "absolute" });
    ...
    $.fn.fadeSlide = function (options) {
        ...
        return this.each(function () {
            ...
            var self = $(this);
            var ul = $("ul", self);
            self.css({ "position": "relative", "display": "block" });
            ul.css({ "position": "relative", "list-style-type": "none" });
            var li = $("li", ul);
            if (li.size() > 1)
                li
					.css({ "top": "0", "left": "0", "position": "absolute", "opacity": "0.0", "display": "none" });

            var controls = $("<div class='controls hide-responsive'></div>").css({ "position": "absolute" });
                ...
                var anchor = $("<a data='" + i + "' href='javascript:void'></a>")
                                .bind("click", function () {
                                    if (li.size() > 1) {
                                        ...
                                        $("ul li:nth-child(" + aindex + ")", self).css("display", "block").animate({ "opacity": "1" }, opts.duration);
                                        $("ul li:nth-child(" + currentIndex + ")", self).animate({ "opacity": "0" }, opts.duration).css("display", "none");
                                        currentIndex = aindex;
                                        nextIndex = aindex;
            ...
            $("ul li:nth-child(" + currentIndex + ")", self).css("display", "block").animate({ "opacity": "1" }, opts.duration);
            ...
            function nextSlide() {
                ...
                $("a[data='" + currentIndex + "']", controls).removeClass("current");
                $("ul li:nth-child(" + nextIndex + ")", self).css("display", "block").animate({ "opacity": "1" }, opts.duration);
                $("ul li:nth-child(" + currentIndex + ")", self).animate({ "opacity": "0" }, opts.duration).css("display", "none");
        ...
        return this.each(function () {
            ...
            function moveSlide() {
                ...
                if (opts.direction == "horizontal") {
                    ...
                    ul.animate({ "left": -posTogo + "px" }, opts.duration);
                }
                else {
                    posTogo = currentIndex * liH;
                    ul.animate({ "top": -posTogo + "px" }, opts.duration);
                }
            ...
            function setPreNext() {
                if (currentIndex + opts.itemsVisible >= lis.size()) next.css({ "display": "none" });
                else next.css({ "display": "block" });
                if (currentIndex == 0) pre.css({ "display": "none" });
   ...
   $.fn.popUp = function (options) {
       ...
       if ($(".bb-overlay").size() == 0)
           overlay = $("<div class='bb-overlay'></div>").css({ "position": "fixed", "left": "0", "top": "0", "width": "100%", "height": "100%", "background": "#000", "opacity": "0.5", "display": "none", "z-index": "999999" }).appendTo("body");
       ...
       var show_dlg = function () {
           ...
           overlay.show();
           dlg
               .css({ "z-index": "99999999", "position": "fixed", "left": "35%", "top": opts.top + "px", "background-color": "#fff", "border": "1px", "border-color": "#000", "padding": "10px" })
               .fadeIn();
       };
       //"margin-left": -dlgW / 2 + "px",
    ...
    $.fn.bbLoading = function (options) {
        ...
        return this.each(function () {
            ...
            var bb_overlay = $("<div class='loading-overlay'></div>")
                    .css({ "position": "absolute", "left": "0", "top": "0", "width": self.width() + "px", "height": h + "px", "zIndex": "10000", "background": "#000", "opacity": opts.opacity, "paddingLeft": x + "px", "paddingTop": y + "px" });
            var loading = $("<i data='loading' class='vloader bb-loading'  ></i>")
                    .css({ "position": "absolute", "top": y + "px", left: x + "px", "zIndex": "10001" });
            self
                .append(bb_overlay)
                .append(loading);
        });
...
angularMain
    //.directive('locationDrop', function () {
    //    return {
    //        restrict:'A',
    //        require: 'ngModel',
    ...
    //                var valid=(parseInt(attr.total) ==1 ) ||(parseInt(attr.total) >1 && value!=0);
    //                ngModel.$setValidity('locationDrop', valid);
    //                return value;
    //            });
    ...
    .directive('lazyImg', function () {
        ...
        //    //    if(prop.charAt(0) != '$') {
        //    //        if (prop == 'ngSrc')
        //    //             attributes += ' data-src="{{' + attrs[prop] + '}}" ';  // special case src attribute into data-src.
    ...
    .directive('productSlide', function () {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                //    console.log('items:' + attrs.items + ',size:' + $("li", paras.container + " " + paras.overview).size());
                //    return  parseInt(attrs.items)>0  && parseInt(attrs.items) == $("li", paras.container + " " + paras.overview).size();
                //}, function (newValue) {
                //    console.log('enter');
                //    if (newValue) {
                //        //wt();
                //        console.log('reload carosel');
                //        $(element).tinycarousel(paras);
                //    }

                //})

                var wt = scope.$watch(attrs.valueToWatch
    ...
    .directive('fadeSlide', function () {
        return {
            ...
            link: function (scope, element, attrs) {
                var wt = scope.$watch(function () {
                    return parseInt(attrs.items) > 0 && parseInt(attrs.items) == $("li", element).size() && $.grep($("li", element), function (n, i) { return !/{{.*?}}/.test($(n).attr("title")); }).length == parseInt(attrs.items);
                }, function (newValue) {
                    if (newValue) {
                        wt();
                        $(element).fadeSlide({ navTitle: true });
                    }
    ...
    .factory('productFac', ['$http', function ($http) {
        return {
            ...
            productSameCatByProductID: function (id, cat) {
                return $http.post('/api/Product/ProductSameCatByProductID/', { ID: id, Cat: cat });
            },
    ...
    .factory('discountFac', ['$http', function ($http) {
        return {
            checkDiscountCode: function (userid, shoppingcartid, code, type, discount) {
                return $http.post('/api/Discount/CheckDiscountCode/', { UserId: userid, ShoppingCartID: shoppingcartid, Code: code, Discount:discount });
            },
            checkDiscountKasCode: function (userid, shoppingcartid, code, type) {
                return $http.post('/api/Discount/CheckDiscountKasCode/', { UserId: userid, ShoppingCartID: shoppingcartid, Code: code });
            },
            CheckDiscountiBookCode: function (userid, shoppingcartid, code, type) {
                return $http.post('/api/Discount/CheckDiscountiBookCode/', { UserId: userid, ShoppingCartID: shoppingcartid, Code: code });
            },
    ...
    .factory('memberFac', ['$http', function ($http) {
        return {
            loginForCheckout: function (name, pass, shoppingcartid) {
                return $http.post('/api/AccountApi/LoginForCheckout/', { 'UserName': name, 'Password': pass, 'ShoppingCartID': shoppingcartid });
            },
