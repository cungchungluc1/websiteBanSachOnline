...
class Product{
    ...
    function add($product) {
        include "./././connection.php";
            $sql="INSERT INTO `product`(`id_product`, `name`, `description`, `price`, `sell`, `id_category`, `description_detail`,'date_publishing', 'id_publishing_company') VALUES (:id_product, :name, :description, :price, :sell, :id_category, :description_detail, :date_publishing, :id_publishing_company)";
    ...
    include "./././connection.php";
    $sql="UPDATE `product` SET `id_category`=:id_product',`id_publishing_company`=:id_publishing_company,`name`=:name,`description`=:description,`price`=:price,`sell`=:sell,`id_category`=:id_category,`date_publishing`=:date_publishing,`description_detail`=:description_detail WHERE 1 `id_product`=:id_product";
    $query = $dbh->prepare($sql);
    $query->bindParam(':id_product',$product->id_product,PDO::PARAM_STR);
    $query->bindParam(':name',$product->name,PDO::PARAM_STR);
    $query->bindParam(':description',$product->description,PDO::PARAM_STR);
